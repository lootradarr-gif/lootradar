...
export function GameCard({ g, i = 0 }: { g: GameWithMarket; i?: number }) {
  const up = g.market.change24h >= 0;
  const preStatus = g.status === 'PRE-TOKEN';   // sadece rozet için
  const noToken = !g.tokenAddress;              // finansal metrik gösterimi (token yoksa $0 yerine —)
  return (
    <Link
      href={`/game/${g.id}`}
      style={{ animationDelay: `${Math.min(i, 12) * 45}ms` }}
      className={`card card-hover reveal-up group relative block overflow-hidden ${g.boosted ? 'ring-1 ring-gold/40 boost-glow' : ''}`}
    >
      {/* ── BANNER ── */}
      <div className="relative h-28 w-full overflow-hidden">
        {g.bannerUrl ? (
          <Image
            src={g.bannerUrl} alt="" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 320px"
        ...
        <div className="absolute right-2.5 top-2.5 flex items-center gap-1.5">
          ...
          <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold backdrop-blur ${preStatus ? 'bg-black/45 text-dim' : 'bg-acc/90 text-white'}`}>
            {preStatus ? 'PRE-TOKEN' : g.status}
          </span>
        </div>
        {/* alt-sol online — sadece GERÇEK veri varsa göster */}
        {g.playersOnline > 0 && (
      ...
      <div className="p-4 pt-3">
        ...
        <div className="mt-3 flex items-end justify-between border-t border-line/70 pt-3">
          <div>
            ...
            <div className="mono mt-0.5 text-base font-bold text-ink">{noToken ? '—' : price(g.market.price)}</div>
          </div>
          <div className="text-right">
            <div className="text-[10px] uppercase tracking-wide text-faint">Market cap</div>
            <div className="mono mt-0.5 text-base font-bold text-ink">{noToken ? '—' : usd(g.market.mcap)}</div>
          </div>
        </div>
        <div className="mt-2 flex items-center justify-between text-[11px] text-faint">
          <span className="mono">{noToken ? (g.rating > 0 ? `★ ${compact(g.rating)} rating` : 'Solana') : (g.holders > 0 ? `${compact(g.holders)} holders` : 'Solana')}</span>
          <span className="mono">{noToken ? 'No token yet' : `${usd(g.market.vol24h)} vol`}</span>
        </div>
      </div>
    </Link>
  );
